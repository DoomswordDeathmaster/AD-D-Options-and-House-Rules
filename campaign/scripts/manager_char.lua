...
function addToArmorDB(nodeItem)
    ...
    Debug.console("manager_char.lua", "_", _, "sSubtypeLower", sSubtypeLower)

    --local bProtOther = ItemManager2.isProtectionOther(nodeItem);

    -- skip the remainder if type is not armor
    ...
    if bIsArmor then
        local armorDp = 0
        --Debug.console("manager_char.lua", "bIsArmor", bIsArmor, "bIsShield", bIsShield)
        Debug.console("manager_char.lua", "nodeItem", nodeItem, "nodeItem Name", DB.getValue(nodeItem, "name", ""))
        local armorItemNameLower = StringManager.trim(DB.getValue(nodeItem, "name", "")):lower()
        Debug.console("manager_char.lua", "armorItemNameLower", armorItemNameLower)
        
        if StringManager.contains(DataCommonADND.aArmorNames, armorItemNameLower) then
            armorDp = DataCommonADND.aDefaultArmorDp[armorItemNameLower][1]
        end

        Debug.console("manager_char.lua", "armorItemNameLower", armorItemNameLower, "armorDp", armorDp)
        DB.setValue(nodeItem, "armor.dp.base", "number", armorDp)
    end
...
function updateHPForLevel(nodeChar, nodeClass, nodeAdvance)
    ...
    Debug.console("manager_char.lua updateHPForLevel", nOriginalHP, nHP, nClassCount, nLevel, sClassName)
    ...
    if nClassCount == 1 then
        ...
        Debug.console("manager_char.lua updateHPForLevel nHPRoll 1", nHPRoll)
        Debug.console("manager_char.lua updateHPForLevel nConBonus 1", nConBonus)
